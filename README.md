...
```
Operator is a tool for building command line interfaces (with Node) that are easy to learn.

## Install
This project is totally experimental so I haven't published it to npm. For now you can do the following:
```bash
...
## Philosophy
Command line tools are easy to use. The problem with them is that they're really hard to _learn_. You need a manual (or a patient instructor) just to get a sense for what a tool can do, and you need to precisely memorize its commands before you can actually use it. It's often said that a tool with a well designed graphical user interface is "easy to use". I think what's usually meant by this is that the tool is easy to learn - GUIs teach people how to use tools while they use them, which mostly eliminates the need for unpleasant manuals and memorization. Operator proposes a way to build command line interfaces that are every bit as easy to learn as their graphical counterparts.